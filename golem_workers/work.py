...
from golem_workers.cluster import Node
...
async def wait_for_http_endpoint(context : WorkContext, *, network_name : str, port : int, path : str = '/', timeout_minutes: float = 15) -> None:
    import websockets
    ...
    async def try_connect():
        try:
            async with websockets.connect(f"{uri}/{port}", additional_headers={
