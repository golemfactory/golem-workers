...
@click.group(name="db", help="Database management.", context_settings={"show_default": True})
def database_cli(): ...


@database_cli.command(help="Generate a new migration file.")
@with_datadir
@click.option(
    "--name",
    type=str,
...
@database_cli.command(help="Show the DB migration files.")
@with_datadir
@click.option(
    "-f",
    "--show-full",
...
@database_cli.command(help="Run all the updates.")
@with_datadir
def upgrade(datadir):
    async def _upgrade():
        async with ReputationService(datadir, auto_apply_migrations=False) as service:
...
@database_cli.command(help="Rollback all the migrations up to and including the given version.")
@with_datadir
