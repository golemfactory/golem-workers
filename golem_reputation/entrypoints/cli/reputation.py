...
from golem_reputation import models
from golem_reputation.entrypoints.cli.utils import with_datadir
from golem_reputation.service import ReputationService
from golem_reputation.updater import ReputationUpdater
...
def list_(datadir, network, node_id, blacklist):
    async def list_records():
        ...
        async with ReputationService(datadir):
            qs = models.NodeReputation.get_blacklisted(blacklist)
            ...
            async for node_reputation in qs:
                node: models.Node = node_reputation.node
                table.add_row(
                    [
                        node.node_id,
...
def block(datadir, network, node_id):
    async def _block():
        async with ReputationService(datadir):
            node, _ = await models.Node.get_or_create(node_id=node_id)
            node_network, _ = await models.Network.get_or_create(network_name=network)
            node_reputation, _ = await models.NodeReputation.get_or_create(
                node=node, network=node_network
            )
            node_reputation.blacklisted_until = models.BLACKLISTED_FOREVER
...
def unblock(datadir, network, node_id):
    async def _unblock():
        async with ReputationService(datadir):
            try:
                node_reputation = await models.NodeReputation.get(
                    node__node_id=node_id, network__network_name=network
                )
                node_reputation.blacklisted_until = models.BLACKLISTED_NEVER
                await node_reputation.save()
